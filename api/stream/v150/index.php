<?php
	...
	foreach($twitch_json->streams as $aStream) {
		...
		$combinedList[$viewers][] ="<tr href='{$link}' data-id='{$id}' class='streams twitch' title='{$title}' rel='tooltip'><td class='vod_date' alt='{$timeStamp}'><span class='iconTwitch'></span></td><td>{$name}</td><td class='textRight'>{$viewers}</td></tr>";
	}
	...
	foreach($owned_json as $aStream) {
		...
		$combinedList[$viewers][] = "<tr href='{$link}' data-id='{$id}' class='streams owned' title='{$title}' rel='tooltip'><td class='vod_date' alt='{$timeStamp}'><span class='iconOwned'></span></td><td>{$name}</td><td class='textRight'>{$viewers}</td></tr>";
	}
	...
	function parseVods($dude, $dudeIcon, $aVOD) {
		...
		$ytList[$timeStamp][] = "<tr href='{$link}' class='vod youtube' title='{$title}' rel='tooltip'><td title='{$dude}' class='vod_date' alt='{$timeStamp}'><span class='{$dudeIcon}'></span></td><td title='{$titleIfNameis2Long}'>{$name}</td><td class='textRight'>{$viewers}</td></tr>";
	}
?>